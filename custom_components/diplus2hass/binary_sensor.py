...
async def async_setup_entry(hass, config_entry, async_add_entities):
    ...
    for signal_key, cfg in BINARY_SENSORS.items():
        sensors.append(DiplusBinarySensor(signal_key, cfg, config_entry, car_name))
    ...
    async def discover_geofences():
        ...
        for key in core.find_geofence_keys(signals):
            ...
            new_entities.append(DiplusBinarySensor(key, cfg, config_entry, car_name))
        if new_entities:
...
class DiplusBinarySensor(BinarySensorEntity, RestoreEntity):
