    ...
    def _coerce_value(self, raw):
        ...
        try:
            ...
            if self._is_numeric:
                ...
                return None
            return raw
        # HA rejects non-finite values for measurement state_class: 'nan' /
        # 'inf' from a source must not reach the state (kept in raw_value).
        import math
        if self._is_numeric and not math.isfinite(value):
            attrs = dict(self._attr_extra_state_attributes)
            attrs["raw_value"] = raw
            self._attr_extra_state_attributes = attrs
            return None
