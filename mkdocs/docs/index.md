...
| Что | Файл | Ссылка |
|-----|------|--------|
| Android-приложение (текущая версия) | `car2hass-v3.3.11.apk` | [скачать](https://mytechnic.ru/cartelemetry/download.php?file=app) |
| Интеграция HA (текущая версия) | `cartelemetry-v3.3.11.zip` | [скачать](https://mytechnic.ru/cartelemetry/download.php?file=integration) |
...
- **Телеметрия в реальном времени** — скорость, запас хода, SOC, температура, двери, окна, шины, свет, зарядка и ещё около 200 сигналов.
...
- **Облако и личный кабинет** — опциональная синхронизация телеметрии с mytechnic.ru (вход по email + TOTP); в кабинете — сенсоры, GPS-трек, файлы настроек.
- **Поддержка Voyah** — канал Voyah CANBus (только чтение) через вендорский сервис.
- **Каналы обновлений** — стабильный и бета-канал для приложения и интеграции.
