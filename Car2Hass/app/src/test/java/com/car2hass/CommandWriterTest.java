...
public class CommandWriterTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        testPreferredOrderFollowsActiveChannels();
        testNoFallbackUsesFirstUsableOnly();

        System.out.println("All CommandWriter tests passed.");
    }

    /** Order comes from the active channels: diplus first → diplus drives the command. */
    private static void testPreferredOrderFollowsActiveChannels() {
        CommandWriter w = writer(java.util.Arrays.asList("diplus", "native"), true,
                okRunner(), diplus(true), verified());
        CommandWriter.Result r = w.dispatch("ac_on", "1");
        assertEquals("diplus", r.usedInterface, "diplus first per active channels");
        assertEquals(0, r.fallbacks, "primary succeeded, no fallback");
    }

    /** allowFallback=false: only the first usable interface is attempted. */
    private static void testNoFallbackUsesFirstUsableOnly() {
        DiplusSenderStub failing = diplus(false);
        CommandWriter w = writer(java.util.Arrays.asList("diplus", "native"), true,
                okRunner(), failing, verified());
        CommandWriter.Result r = w.dispatch("ac_on", "1");
        assertEquals(true, r.success, "native fallback still succeeds when primary rejected");
        // With fallback disabled the second interface must not be tried.
        CommandWriter w2 = writer(java.util.Arrays.asList("diplus", "native"), false,
                okRunner(), failing, verified());
        CommandWriter.Result r2 = w2.dispatch("ac_on", "1");
        assertEquals(false, r2.success, "no fallback -> failure reported");
    }
    ...
    private static CommandWriter writer(List<String> preferredIfaces, boolean allowFallback,
                                        ScriptedRunner runner, DiplusSenderStub diplus,
                                        VerifierStub verifier) {
        NativeCommandWriter nativeWriter = new NativeCommandWriter(runner, "127.0.0.1", 5555);
        return new CommandWriter(nativeWriter, diplus, verifier, testMessages(),
                preferredIfaces, allowFallback, false);
    }
