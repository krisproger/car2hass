...
public class CommandWriter {
    ...
    /** Legacy source string ("auto"/"native"/"diplus"); used when no channel list is given. */
    private final String telemetrySource;
    /** Interfaces in priority order (values: IFACE_NATIVE / IFACE_DIPLUS). */
    private final List<String> preferredIfaces;
    private final boolean allowFallback;
    private boolean lastNativeWasOk;

    /**
     * Preferred-interface constructor: order follows the active telemetry
     * channels (the channel that supplies the sensor also drives the command),
     * {@code allowFallback}=false keeps only the first usable interface.
     */
    public CommandWriter(NativeCommandWriter nativeWriter, DiplusSender diplusSender,
                         Verifier verifier, Messages messages,
                         List<String> preferredIfaces, boolean allowFallback,
                         boolean lastNativeWasOk) {
        this.nativeWriter = nativeWriter;
        this.diplusSender = diplusSender;
        this.verifier = verifier;
        this.messages = messages;
        this.telemetrySource = sourceFromIfaces(preferredIfaces);
        this.preferredIfaces = new ArrayList<>(preferredIfaces != null ? preferredIfaces : new ArrayList<>());
        this.allowFallback = allowFallback;
        this.lastNativeWasOk = lastNativeWasOk;
    }

    private static String sourceFromIfaces(List<String> ifaces) {
        if (ifaces == null || ifaces.isEmpty()) return SRC_AUTO;
        return ifaces.contains(IFACE_NATIVE) && !ifaces.contains(IFACE_DIPLUS)
                ? SRC_NATIVE : SRC_DIPLUS;
    }

    /** Legacy constructor: derives the order from the saved telemetry source. */
    public CommandWriter(NativeCommandWriter nativeWriter, DiplusSender diplusSender,
                         Verifier verifier, Messages messages,
        ...
        this.preferredIfaces = new ArrayList<>();
    }
    ...
    /** Orders interfaces: preferred list first; legacy source string as fallback. */
    private String[] interfaceOrder(boolean nativeOk, boolean diplusOk) {
        if (!preferredIfaces.isEmpty()) {
            List<String> out = new ArrayList<>();
            for (String iface : preferredIfaces) {
                boolean ok = IFACE_NATIVE.equals(iface) ? nativeOk : diplusOk;
                if (!ok) continue;
                out.add(iface);
