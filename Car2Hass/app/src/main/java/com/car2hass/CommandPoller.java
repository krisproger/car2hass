...
public class CommandPoller {
    ...
    private boolean pollOnce() {
        ...
        try {
            ...
            pollUrl = String.format(Locale.US, "%s://%s:%d/api/cartelemetry/commands?car_name=%s",
    ...
    private boolean acknowledgeCommand(String commandId, String status, String message) {
        ...
        String scheme = https ? "https" : "http";
        // The HA cartelemetry commands view is registered on /api/cartelemetry/commands
        // and handles both GET (poll) and POST (ack) on the same URL.
        String ackUrl = String.format(Locale.US, "%s://%s:%d/api/cartelemetry/commands",
            scheme, host, port);
