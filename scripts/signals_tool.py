...
JAVA_READER = ROOT / "Car2Hass" / "app" / "src" / "main" / "java" / "com" / "car2hass" / "CANDataReader.java"
SIGNAL_TRANSLATOR = ROOT / "Car2Hass" / "app" / "src" / "main" / "java" / "com" / "car2hass" / "SignalTranslator.java"
CONST_PY = ROOT / "custom_components" / "cartelemetry" / "const.py"
...
def cmd_gen_md(args):
    ...
    for section, rows in sections.items():
        ...
        for s in rows:
            if s.get("synthetic"):
                continue  # app-supplied signals are not DiPlus getVal entries
...
def cmd_gen_spec(args):
    """Generate the open API spec (docs/cartelemetry/api/spec/spec.json)
    from the JSON registries: universal core sensors, extended sensors,
    commands and the telemetry payload schema."""
    import datetime
    import json
    ...
    extended = [sensor_entry(s) for s in sensors["sensors"] if not s.get("core")]

    cmds = []
    for c in commands["commands"]:
        cmds.append({
            "id": c["id"],
            "state_sensor": c.get("state_sensor"),
            "param": c.get("param"),
            "channels": sorted(k for k, v in c.get("channels", {}).items() if v is not None),
        })

    spec = {
        "spec_version": 1,
        "generated": datetime.datetime.now(datetime.timezone.utc).isoformat(timespec="seconds"),
        "integration_version": _const_version(),
        "endpoint": {
    ...
    out = ROOT / "docs" / "cartelemetry" / "api" / "spec" / "spec.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(spec, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"spec: core={len(core)} extended={len(extended)} commands={len(cmds)} -> {out}")


def _const_version():
    import re
    text = (ROOT / "custom_components" / "cartelemetry" / "const.py").read_text(encoding="utf-8")
    m = re.search(r'INTEGRATION_VERSION\s*=\s*"([^"]+)"', text)
...
def main():
    ...
    sub.add_parser("gen-spec", help="Generate open API spec JSON for the site")
